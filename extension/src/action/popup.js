...
btns.forEach((btn, index) => {
	btn.addEventListener('click', () => { sharebtnonclick(btn.id) })
})
...
const blueskypost = (text, url) => {
	...
	fetch(`https://sparebeatter.vercel.app/api/shorturl/?url=${longurl}`, { method: 'GET' })
		.then((data) => data.json())
		.then((data) => {
			window.open(`https://bsky.app/intent/compose?text=${text}%0D%0A%20%23sparebeat%20%23sparebeatter%20+%0D%0A${data.url}%20`)
		})
}
