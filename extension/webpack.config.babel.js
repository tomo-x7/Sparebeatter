...
export default (minify = false, isfirefox = false) => {
	return {
		...
		entry: {
			...
			embedScripts: path.join(__dirname, "src", "scripts", "content", "embedScripts.ts"),
		...
		plugins: [
			new CopyWebpackPlugin({
				patterns: [
					isfirefox === true
						? {
								from: path.join(__dirname, "src", "manifest.firefox.json"),
								to: path.join(__dirname, "dist", "manifest.json"),
							}
						: {
								from: path.join(__dirname, "src", "manifest.chrome.json"),
								to: path.join(__dirname, "dist", "manifest.json"),
							},
					{
